...
| S0-007 | Make the installed Windows Workbench review build independently identifiable through an approved read-only path | Sprint 0 baseline observability / Windows bridge | Before owner observation, Workbench can verify the installed desktop build/version (and preferably release-equivalent checksum) without relying on owner transport or assumption | Current approved direct Windows-host controls expose host/tool state but not the installed Workbench desktop binary/version/hash. A bounded read-only supervised operations fallback was attempted during Sprint 0 baseline establishment but had not produced verifiable build evidence, so the owner observation gate cannot yet open | `docs/SPRINT_GOVERNANCE.md`; `docs/sprints/SPRINT_00_EXISTING_CORRECTIONS_BASELINE_STABILISATION.md`; current private control capability boundary | P0 — blocks baseline observation gate | NO — baseline observability gap | NO | NOT STARTED | UNVERIFIED | NOT OBSERVED |
