...
func executePrivateSafeHands(ctx context.Context, env privateControlEnvelope, mcpURL, authFile string) (map[string]any, error) {
	...
	if env.Action == "run_override_rin_canonical_export" {
		if env.Project != "" {
			return nil, errors.New("run_override_rin_canonical_export does not accept a project")
		}
		var a struct {
			HostID string `json:"host_id"`
		}
		...
		job, err := core.SubmitOverrideRinCanonicalExportJob(strings.TrimSpace(a.HostID))
		if err != nil {
			return nil, err
		}
		return map[string]any{"host_job": job}, nil
	...
	if env.Action == "run_override_rin_wardrobe_inventory" {
		if env.Project != "" {
			return nil, errors.New("run_override_rin_wardrobe_inventory does not accept a project")
		}
		var a struct {
			HostID string `json:"host_id"`
		}
		...
		job, err := core.SubmitOverrideRinWardrobeInventoryJob(strings.TrimSpace(a.HostID))
		if err != nil {
			return nil, err
		}
		return map[string]any{"host_job": job}, nil
	...
	if env.Action == "run_override_rin_field_outfit_inspect" {
		if env.Project != "" {
			return nil, errors.New("run_override_rin_field_outfit_inspect does not accept a project")
		}
		var a struct {
			HostID string `json:"host_id"`
		}
