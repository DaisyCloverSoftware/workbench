...
func TestRelayInstallerSmokeDoesNotConsumeLiveQueue(t *testing.T) {
	text := installerScript(t, "install-github-relay.sh")
	if !strings.Contains(text, `"$bin_dir/workbench-relay" "${relay_args[@]}" --smoke-only`) {
		t.Fatal("relay installer must use the non-consuming smoke-only mode before supervisor restart")
	}
