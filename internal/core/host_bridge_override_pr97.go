...
func validateOverridePR97Host(host HostBridgeHost, now time.Time) error {
 ...
 for _, field := range strings.Fields(capability.Version) {
  if field == overridePR97Capability { supported = true }
 }
...
func SubmitOverridePR97ProofJob(hostID string) (HostJob, error) {
 ...
 if err != nil || !strings.HasPrefix(hostID, "windows_") { return HostJob{}, errors.New("PR97 proof requires a Windows host id") }
 var job HostJob
 err = withHostBridgeLock(func(root string) error {
  var host HostBridgeHost
  if err := readHostBridgeJSON(filepath.Join(root,"hosts",hostID+".json"), &host); err != nil { return err }
  if host.HostID != hostID { return errors.New("PR97 host identity mismatch") }
  if err := validateOverridePR97Host(host,time.Now().UTC()); err != nil { return err }
  entries, err := os.ReadDir(filepath.Join(root,"jobs")); if err != nil { return err }
  for _, entry := range entries {
   if entry.IsDir() || !strings.HasSuffix(entry.Name(),".json") { continue }
   var existing HostJob
   if err := readHostBridgeJSON(filepath.Join(root,"jobs",entry.Name()), &existing); err != nil { return err }
  ...
  id, err := newHostJobID(); if err != nil { return err }
...
type overridePR97NativeReport struct {
 ...
 Build struct { Skipped bool `json:"skipped"`; ExitCode *int `json:"exitCode"` } `json:"build"`
 Automation struct { Skipped bool `json:"skipped"`; ExitCode *int `json:"exitCode"` } `json:"automation"`
}
...
type overridePR97PackageReport struct {
 ...
 Success bool `json:"success"`
 Error string `json:"error"`
 Package struct { ExitCode *int `json:"exitCode"`; Executable string `json:"executable"` } `json:"package"`
 RuntimeProof struct { MapReachedPlay bool `json:"mapReachedPlay"` } `json:"runtimeProof"`
}

func decodeOverridePR97NativeReport(raw []byte) (overridePR97NativeReport,error) {
 var report overridePR97NativeReport
 if len(raw)==0 || len(raw)>1<<20 { return report,errors.New("PR97 native evidence missing or oversized") }
 if err := json.Unmarshal(raw,&report); err != nil { return report,err }
...
func decodeOverridePR97PackageReport(raw []byte) (overridePR97PackageReport,error) {
 ...
 if len(raw)==0 || len(raw)>1<<20 { return report,errors.New("PR97 packaged evidence missing or oversized") }
 if err := json.Unmarshal(raw,&report); err != nil { return report,err }
...
type overridePR97LFSCapture struct { line []byte; count int; bad bool }
func (c *overridePR97LFSCapture) Write(p []byte) (int,error) {
 for _,b := range p {
  if b=='\n' { c.checkLine(); continue }
  if len(c.line)>=4096 { c.bad=true; continue }
  c.line=append(c.line,b)
 }
 return len(p),nil
}
func (c *overridePR97LFSCapture) checkLine() {
 line:=strings.TrimSpace(string(c.line));c.line=c.line[:0]
 if line=="" { return }
 fields:=strings.Fields(line)
 if len(fields)<3 || fields[1]!="*" { c.bad=true }
 c.count++
}
func (c *overridePR97LFSCapture) complete() bool {
 if len(c.line)>0 { c.checkLine() }
