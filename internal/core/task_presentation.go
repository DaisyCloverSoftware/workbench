...
func PresentTask(t Task) TaskPresentation {
	...
	case TaskCompleted:
		...
		case p.ReviewBranch != "" && p.PublicationStatus == ReviewPublicationFailed:
			p.NextAction = fmt.Sprintf("Code is ready for review on %s at %s. Automatic publication did not complete; retry review delivery without recoding.", p.ReviewBranch, shortCommit(p.ReviewCommit))
