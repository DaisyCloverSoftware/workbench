...
func executeWindowsHostBridgeJob(ctx context.Context, hostID string, job HostJob) (HostJobResult, string) {
	...
	case HostBridgeToolWorkbench:
		...
		case HostBridgeOperationOverrideRinFieldOutfitInspect:
			inspectCtx, cancel := context.WithTimeout(ctx, overrideRinFieldOutfitInspectTimeout)
			defer cancel()
			output, err := runOverrideRinFieldOutfitInspect(inspectCtx, job.ID)
			if err != nil {
				return HostJobResult{ExitCode: 1}, err.Error()
			}
			return HostJobResult{Output: output, ExitCode: 0}, ""
		case HostBridgeOperationOverrideRinFieldOutfitReviewCapture:
			reviewCtx, cancel := context.WithTimeout(ctx, overrideRinFieldOutfitReviewTimeout)
			defer cancel()
			output, err := runOverrideRinFieldOutfitReviewCapture(reviewCtx, job.ID)
			if err != nil {
				return HostJobResult{ExitCode: 1}, err.Error()
			}
			return HostJobResult{Output: output, ExitCode: 0}, ""
