...
// SubmitBlenderSmokeRenderJob is deliberately separate from the generic typed
// host-job submitter. The generic submitter remains version-only; this function
// can create exactly one additional Blender operation with no caller-supplied
// path, file, arguments, script, scene or output destination.
func SubmitBlenderSmokeRenderJob(hostID string) (HostJob, error) {
	...
	job := HostJob{
		...
		HostID: hostID,
		Spec: HostJobSpec{
			Tool:      HostBridgeToolBlender,
			Operation: HostBridgeOperationBlenderSmokeRender,
		},
		Status:    "queued",
		CreatedAt: now,
		UpdatedAt: now,
...
func blenderSmokeRenderInvocation(executable, outputPrefix string) (string, []string, error) {
	...
	if outputPrefix == "" || strings.ContainsAny(outputPrefix, "\r\n\x00") {
		return "", nil, errors.New("Blender smoke render output path is invalid")
	}
	if !filepath.IsAbs(outputPrefix) || filepath.Base(outputPrefix) != "smoke_" {
		return "", nil, errors.New("Blender smoke render output path is outside the fixed Workbench shape")
	}
	return name, []string{
		"--background",
		"--factory-startup",
...
func verifyBlenderSmokeRender(path string) (string, error) {
	...
	if err != nil {
		return "", errors.New("Blender smoke render did not produce the expected PNG")
	}
	if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
		return "", errors.New("Blender smoke render output is not a regular file")
	}
	if info.Size() <= 0 || info.Size() > maxBlenderSmokeRenderBytes {
		return "", errors.New("Blender smoke render output size is outside the allowed range")
	}
	f, err := os.Open(path)
	if err != nil {
		return "", errors.New("Blender smoke render output could not be verified")
	}
	defer f.Close()

	signature := make([]byte, len(blenderSmokePNGSignature))
	if _, err := io.ReadFull(f, signature); err != nil || string(signature) != string(blenderSmokePNGSignature) {
		return "", errors.New("Blender smoke render output is not a PNG")
	}
	if _, err := f.Seek(0, io.SeekStart); err != nil {
		return "", errors.New("Blender smoke render output could not be verified")
	}
	h := sha256.New()
	n, err := io.Copy(h, io.LimitReader(f, maxBlenderSmokeRenderBytes+1))
	if err != nil || n != info.Size() || n > maxBlenderSmokeRenderBytes {
		return "", errors.New("Blender smoke render output changed while being verified")
	}
