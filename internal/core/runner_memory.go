...
func BuildWorkerPromptFromStoredKnowledge(task Task) string {
	knowledgeProject := taskKnowledgeProject(task)
	found, _ := SearchKnowledge(knowledgeProject, task.Intent, 16)
	...
	if c, ok, err := LatestContextCapsule(knowledgeProject); err == nil && ok {
		capsule = &c
	}
