...
func TestPrepareChangesetPreservesExecutableBit(t *testing.T) {
	...
	if prepared.Fingerprint != before.Fingerprint {
		t.Fatalf("prepared fingerprint=%s want=%s", prepared.Fingerprint, before.Fingerprint)
	}
	if got := prepareTestGit(t, repo, "ls-tree", prepared.Commit, "tracked.txt"); !strings.HasPrefix(got, "100755 blob ") {
		t.Fatalf("prepared file mode was not executable: %q", got)
	}
}
