...
func toolsList() []map[string]any {
	...
	return []map[string]any{
		...
		tool("delegate_operation", "Delegate optional autonomous machine operation", "Optional fallback only: delegate a machine-side outcome to an external operator when the required work cannot be expressed through inspect_machine/run_machine_command and autonomous operator capacity is intentionally available. Do not use this for routine cluster commands, source code, Git/GitHub changes, pull requests, CI or GitHub Actions.", objSchema(map[string]any{"intent": strProp("Machine-side operational outcome to achieve"), "project_path": project}, []string{"intent"}), anyObjectSchema(), annotations(false, true, true)),
