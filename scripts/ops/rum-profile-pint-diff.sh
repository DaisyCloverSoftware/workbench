...
FILES=(
  "apps/api/app/Http/Controllers/Api/V1/ProfileController.php"
  "apps/api/app/Services/ProfileService.php"
  "apps/api/app/Services/Reputation/ReputationCalculationService.php"
)
...
"$RUNTIME" exec --user 0:0 -w /work/apps/api "$cid" sh -lc \
  'composer install --no-interaction --prefer-dist --no-progress >/dev/null && vendor/bin/pint app/Http/Controllers/Api/V1/ProfileController.php app/Services/ProfileService.php app/Services/Reputation/ReputationCalculationService.php >/dev/null'
for file in "${FILES[@]}"; do
  "$RUNTIME" cp "$cid:/work/$file" "$tmp/rum/$file"
done

printf '%s\n' '--- PROFILE PINT DIFF START ---'
git -C "$tmp/rum" diff -- "${FILES[@]}"
printf '%s\n' '--- PROFILE PINT DIFF END ---'
