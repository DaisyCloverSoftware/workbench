...
for command in kubectl sleep mktemp; do
...
kubectl get namespace "$DEV_NS" >/dev/null
kubectl get namespace "$LIVE_NS" >/dev/null
kubectl -n "$DEV_NS" get secret "$PULL_SECRET" >/dev/null
kubectl -n "$DEV_NS" get deployment "$DEPLOYMENT" >/dev/null
kubectl -n "$LIVE_NS" get deployment "$DEPLOYMENT" >/dev/null

dev_image_before="$(kubectl -n "$DEV_NS" get deployment "$DEPLOYMENT" -o jsonpath='{.spec.template.spec.containers[0].image}')"
live_image_before="$(kubectl -n "$LIVE_NS" get deployment "$DEPLOYMENT" -o jsonpath='{.spec.template.spec.containers[0].image}')"
...
cleanup() {
  if [ "$created" = true ]; then
    kubectl -n "$DEV_NS" delete pod "$probe_pod" --ignore-not-found --wait=false >/dev/null 2>&1 || true
  fi
  rm -f "$manifest"
}
trap cleanup EXIT HUP INT TERM

if kubectl -n "$DEV_NS" get pod "$probe_pod" >/dev/null 2>&1; then
...
for ((attempt = 0; attempt < 90; attempt++)); do
  phase="$(kubectl -n "$DEV_NS" get pod "$probe_pod" -o jsonpath='{.status.phase}' 2>/dev/null || true)"
  waiting_reason="$(kubectl -n "$DEV_NS" get pod "$probe_pod" -o jsonpath='{.status.containerStatuses[0].state.waiting.reason}' 2>/dev/null || true)"
  ...
  case "$waiting_reason" in
    ErrImagePull|ImagePullBackOff|InvalidImageName)
      ...
      kubectl -n "$DEV_NS" describe pod "$probe_pod" >&2 || true
      exit 5
      ;;
  esac
...
image_id="$(kubectl -n "$DEV_NS" get pod "$probe_pod" -o jsonpath='{.status.containerStatuses[0].imageID}')"
digest="${image_id##*@}"
...
kubectl -n "$DEV_NS" delete pod "$probe_pod" --wait=true --timeout=30s >/dev/null
created=false

dev_image_after="$(kubectl -n "$DEV_NS" get deployment "$DEPLOYMENT" -o jsonpath='{.spec.template.spec.containers[0].image}')"
live_image_after="$(kubectl -n "$LIVE_NS" get deployment "$DEPLOYMENT" -o jsonpath='{.spec.template.spec.containers[0].image}')"
